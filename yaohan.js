		...
		packageCodeList1.forEach((item,index)=>{
        	packageCodeList2.forEach((item2,index)=>{
		        packageCodeList3.forEach((item3,index)=>{
                    ...
                    var ex = item+'-'+item2+'-'+item3
                    // 数据项
                    newArr.push({
                        prodspec:'全部发布范围',//商品规格
                        customspec:2,//自定义规格
                        price:'',//单价（元）
                        stocknum:'',//库存数量（件）
                        specscode:'',//规格码
            ...
            [
                {
                    ...
                    //width: 50,
                    formatter: function (value, row, index) {
                    return `<select class="selectpicker" onchange="set_prodspec(${value, row, index})">
                ...
                },
                {
                    ...
                    valign: "middle",
                    formatter: function (value, row, index) {
                        return `<input  placeholder="请输入单价(元)" onchange="set_price(${value, row, index})">`;
                    }
                },
                {
                    field: "stocknum",
                    ...
                    valign: "middle",
                    formatter: function (value, row, index) {
                        return `<input  placeholder="请输入库存数量" onchange="set_stocknum(${value, row, index})">`;
                    }
                },
                {
                    field: "specscode",
                    ...
                    valign: "middle",
                     formatter: function (value, row, index) {
                        return `<input  placeholder="请输入商品码" onchange="set_specscode(${value, row, index})">`;
                    }
                },
                {
                    field: "auditStatus",
                    ...
                    valign: "middle",
                    formatter: function (value, row, index) {
                   return `<span  onchange="del_auditStatus(${value, row, index})">删除</span>
                            <span  onchange="copy_auditStatus(${value, row, index})">复制</span>`;
                    } 
                }
...
// 表格选项设置
function set_prodspec(value, row, index){
    console.log(value, row, index)
    newArr[index].customspec = this.value
    // 然后刷新表格，其他的同理
}
function set_price(value, row, index){
    console.log(value, row, index)
    newArr[index].price = this.value
    // 然后刷新表格，其他的同理
}
function set_stocknum(value, row, index){
    console.log(value, row, index)
    newArr[index].stocknum = this.value
    // 然后刷新表格，其他的同理
}
function set_specscode(value, row, index){
    console.log(value, row, index)
    newArr[index].specscode = this.value
    // 然后刷新表格，其他的同理
}
function del_auditStatus(value, row, index){
    console.log(value, row, index)
    newArr.splice(index,1)
    // 刷新table
}
function copy_auditStatus(value, row, index){
    console.log(value, row, index)
    newArr.push(row)
    // 刷新table
}
